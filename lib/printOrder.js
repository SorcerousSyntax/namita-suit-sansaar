    ...
    const rows = items.map((item) => {
        ...
        return `
            <tr>
                ...
                <td>â‚¹${(Number(item.price) || 0).toLocaleString('en-IN')}</td>
                <td>â‚¹${lineTotal.toLocaleString('en-IN')}</td>
            </tr>
        `;
    }).join('');
...
<body>
    ...
    <div class="totals">
        <div class="totals-row"><span>Subtotal</span><span>â‚¹${subtotal.toLocaleString('en-IN')}</span></div>
        <div class="totals-row"><span>Shipping</span><span>${shipping === 0 ? 'FREE' : `â‚¹${shipping.toLocaleString('en-IN')}`}</span></div>
        <div class="totals-row total"><span>Total</span><span>â‚¹${total.toLocaleString('en-IN')}</span></div>
    </div>
